...
1. ⬜ **OpenRouter key — OWNS IT (issue #003, the #1 blocker).** Procure/obtain the key, put it in root `.env` as `OPENROUTER_API_KEY` (optionally `OPENROUTER_API_KEY_FALLBACK`), restart the API, and confirm `curl localhost:4000/api/health` → `providers.openrouter: true`. Ismail approves any spend.
...
|---|---|---|---|---|
| B1 | ~~SEV-3~~ ✅ | api | ~~**Dead conversational-proposal code path.**~~ **Removed (2026-06-13):** deleted `agent-booking.service.ts`, the `POST /api/appointments/confirm` route, client `confirmAgentBooking`/`AgentBookingProposal`, the `booking_proposal` stream event, and the dead `agentProposal` card in `Chat.tsx`. Chat booking goes solely through the `<<BOOK>>` marker → `POST /api/appointments`. typecheck + build green. | Done. |
