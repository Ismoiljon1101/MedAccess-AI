...
| B5 | DOC | docs | **Vision is now sidecar-only, not "hybrid".** `vision.ts` sends images ONLY to the local Python sidecar; the LLM just narrates the sidecar's text findings (no independent cloud-vision read). README, `docs/architecture/image-pipeline.md`, and deck slide 9 still claim "generalist LLM **plus** specialist, show both, flag disagreements." | Update those docs to describe the sidecar-only pipeline (intentional per v1.0 design) — or restore a second LLM read if the "both reads" story is wanted. |
| B6 | DOC | docs | **Image analysis does NOT degrade gracefully.** README/architecture say "Node degrades to LLM-only if the sidecar is unset." In reality `analyzeImageFull` throws **503** when `IMAGE_ML_URL` is missing/unreachable — image upload hard-fails. This is the known ARM64 demo risk (run the sidecar on a non-ARM box). | Fix the wording, or add a true LLM-only fallback path when the sidecar is down. |
